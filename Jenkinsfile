...
pipeline{
	...
	stages {
			...
			steps {
				script {
					def vars = checkout(scm: [$class: 'GitSCM',
						branches: [[name: 'main']],
						userRemoteConfigs: [[url: 'https://github.com/mechatrax/pi-field']]
					], poll: true)
					def msg = sh(
		...
		stage("Upload") {
			steps {
				sh 'curl -sS -T ${TEMP_DIR}/${RELEASE_NAME}.img.xz -u ${FTP_CREDENTIALS_USR}:${FTP_CREDENTIALS_PSW} ftp://${FTP_IP}/data/pi-field/'
				}
			}
		}
		stage("Check") {
	...
	post {
		success {
			zulipSend message: "✔ Pi-field の SD イメージ ${RELEASE_NAME} のリリースに成功しました", stream: "${MAIN_STREAM}"
		}
		failure {
			zulipSend message: "❌ Pi-field の SD イメージ ${RELEASE_NAME} のリリースに失敗しました", stream: "${MAIN_STREAM}"
		}
	}
}
